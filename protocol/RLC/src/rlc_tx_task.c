...
void rlc_tx_process(void *message, MessagesIds      msg_type)
{
	...
	switch (msg_type)
	{
		...
		case IP_RLC_DATA_IND:
		{
			...
			if (rrc_rlc_data_ind_ptr->data_sn > 109)
            {
				g_rlc_debug = 1; 
            }	       
...
cur process time = [%lld, %lld, %lld] \n",
            ...
            AssertFatal(g_rlc_protocol_ctxt.enb_flag == 1, RLC, "g_rlc_protocol_ctxt.enb_flag has been changed by program\n"); 
			break; 
		}
